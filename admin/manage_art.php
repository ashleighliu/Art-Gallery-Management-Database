				...
				<form action="" id="manage-art">
					...
					<div class="form-group row">
						<div class="col-md-5">
							...
							<select name="artist_id" id="" required="" class="custom-select select2">
								<option value=""></option>
								<?php
									$artist = $conn->query("SELECT * FROM users where user_type = 2 order by name asc");
					...
					<div class="form-group row">
						<div class="col-md-10">
							...
							<textarea name="art_description" id="art_description" class="form-control jqte" cols="30" rows="5" required><?php echo isset($art_description) ? html_entity_decode($art_description) : '' ?></textarea>
						</div>
					</div>
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					...
					setTimeout(function(){
						location.href = "index.php?page=arts"
					},1500)
